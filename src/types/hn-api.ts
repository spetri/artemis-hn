...
export type HackerNewsStory = Pick<
  HackerNewsItemBase,
  'by' | 'descendants' | 'id' | 'kids' | 'score' | 'time' | 'title' | 'url' | 'deleted' | 'dead'
> & {
  type: 'story';
};
...
export type HackerNewsPollOpt = Pick<
  HackerNewsItemBase,
  'by' | 'id' | 'poll' | 'score' | 'time' | 'title' | 'text' | 'deleted' | 'dead'
> & {
  type: 'pollopt';
};
...
export type StoryFilters = 'home' | 'top' | 'new' | 'best' | 'show' | 'ask' | 'job';
